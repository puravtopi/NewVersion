...
namespace PainTrax.Web.Controllers
{
    ...
    public class AppProviderController : Controller
    {
        ...
        public IActionResult ImportData(IFormFile postedFile)
        {
            try
            {
                if (postedFile != null && postedFile.Length > 0)
                {
                    ...
                    {
                        if (!string.IsNullOrEmpty(dt.Rows[i]["provider_name"].ToString()))
                        {
                            tbl_app_provider obj = new tbl_app_provider()
                            {
                                cmp_id = cmpid,
                                provider_name = dt.Rows[i]["provider_name"].ToString(),
        ...
        {
            var path = Path.Combine(Environment.WebRootPath + "/Uploads/Sample", "AppProvider.xlsx");
            var fs = new FileStream(path, FileMode.Open);

            // Return the file. A byte array can also be used instead of a stream
            return File(fs, "application/octet-stream", "SampleAppProvider.xlsx");
        }
        ...
        public IActionResult ExportToExcel()
        {
            try
            {
                ...
                {
                    new DataColumn("Provider Name", typeof(string))

                });

                // Populate the DataTable with data from the list of attorneys
                foreach (var appprovider in data)
                {
                    dt.Rows.Add(appprovider.provider_name);
                }
                ...
                using (var document = SpreadsheetDocument.Create(memoryStream, SpreadsheetDocumentType.Workbook))
                {
                    ...
                    var sheet = new Sheet() { Id = document.WorkbookPart.GetIdOfPart(worksheetPart), SheetId = 1, Name = "Appointment Provider" };
                    sheets.Append(sheet);
                ...
                return File(memoryStream.ToArray(), "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", "AppProvider.xlsx");
            }
            catch (Exception ex)
            {
