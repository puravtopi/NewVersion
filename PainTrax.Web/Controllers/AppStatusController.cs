...
namespace PainTrax.Web.Controllers
{
    ...
    public class AppStatusController : Controller
    {
        ...
        {
            var path = Path.Combine(Environment.WebRootPath + "/Uploads/Sample", "AppStatus.xlsx");
            var fs = new FileStream(path, FileMode.Open);

            // Return the file. A byte array can also be used instead of a stream
            return File(fs, "application/octet-stream", "SampleAppStatus.xlsx");
        }
        ...
        public IActionResult ExportToExcel()
        {
            try
            {
                ...
                foreach (var appstatus in data)
                {
                    dt.Rows.Add(appstatus.status);
                }
                ...
                using (var document = SpreadsheetDocument.Create(memoryStream, SpreadsheetDocumentType.Workbook))
                {
                    ...
                    var sheet = new Sheet() { Id = document.WorkbookPart.GetIdOfPart(worksheetPart), SheetId = 1, Name = "AppStatus" };
                    sheets.Append(sheet);
                ...
                return File(memoryStream.ToArray(), "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", "AppStatus.xlsx");
            }
            catch (Exception ex)
            {
