...
namespace PainTrax.Web.Controllers
{
    ...
    public class AttorneysController : Controller
    {
        ...
        public AttorneysController(IMapper mapper, ILogger<AttorneysController> logger, Microsoft.AspNetCore.Hosting.IHostingEnvironment environment)
        {
            _mapper = mapper;
            _logger = logger;
        ...
        {
            tbl_attorneys obj = new tbl_attorneys();
            return View(obj);
        }
        [HttpPost]
        [AllowAnonymous]
        public IActionResult Create(tbl_attorneys model)
        {
            try
            {
        ...
        {
            tbl_attorneys data = new tbl_attorneys();
            try
            {
                tbl_attorneys obj = new tbl_attorneys();
                obj.Id = id;
        ...
        [HttpPost]
        public IActionResult Edit(tbl_attorneys model)
        {
            // model.updatedby = 1;
        ...
        public IActionResult List()
        {
            try
            {
                ...
                {
                    var property = typeof(tbl_attorneys).GetProperties()[Convert.ToInt32(sortColumn)];
        ...
        public IActionResult ImportData(IFormFile postedFile)
        {
            try
            {
                if (postedFile != null && postedFile.Length > 0)
                {
                    ...
                    {
                        if (!string.IsNullOrEmpty(dt.Rows[i]["Attorney"].ToString()))
                        {
                            tbl_attorneys obj = new tbl_attorneys()
                            {
                                cmp_id = cmpid,
                                Attorney = dt.Rows[i]["Attorney"].ToString(),
        ...
        {
            var path = Path.Combine(Environment.WebRootPath + "/Uploads/Sample", "Attorney.xlsx");
            var fs = new FileStream(path, FileMode.Open);

            // Return the file. A byte array can also be used instead of a stream
            return File(fs, "application/octet-stream", "Attorney.xlsx");
        }
        ...
        public IActionResult ExportToExcel()
        {
            try
            {
                ...
                foreach (var attorney in data)
                {
                    dt.Rows.Add(attorney.Attorney, attorney.Address, attorney.City, attorney.State, attorney.Zip, attorney.ContactName, attorney.ContactNo, attorney.EmailId, attorney.CreatedDate);
                }
                ...
                using (var document = SpreadsheetDocument.Create(memoryStream, SpreadsheetDocumentType.Workbook))
                {
                    ...
                    var sheet = new Sheet() { Id = document.WorkbookPart.GetIdOfPart(worksheetPart), SheetId = 1, Name = "Attorneys" };
                    sheets.Append(sheet);
                ...
                return File(memoryStream.ToArray(), "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", "Attorneys.xlsx");
            }
            catch (Exception ex)
            {
