...
namespace PainTrax.Web.Controllers
{
    public class CaseTypeController : Controller
    {
        ...
        private readonly CaseTypeService _services = new CaseTypeService();
        private Microsoft.AspNetCore.Hosting.IHostingEnvironment Environment;

        public CaseTypeController(IMapper mapper, ILogger<CaseTypeController> logger, Microsoft.AspNetCore.Hosting.IHostingEnvironment environment)
        {
            _mapper = mapper;
            _logger = logger;
        ...
        {
            tbl_casetype obj = new tbl_casetype();
            return View(obj);
        }
        [HttpPost]
        [AllowAnonymous]
        public IActionResult Create(tbl_casetype model)
        {
            try 
            {
        ...
        {
            tbl_casetype data = new tbl_casetype();
            try
            {
                tbl_casetype obj = new tbl_casetype();
                obj.Id = id;
        ...
        [HttpPost]
        public IActionResult Edit(tbl_casetype model)
        {
            try
            {
        ...
        public IActionResult List()
        {
            try
            {
                ...
                {
                    var property = typeof(tbl_casetype).GetProperties()[Convert.ToInt32(sortColumn)];
        ...
        public IActionResult ImportData(IFormFile postedFile)
        {
            try
            {
                if (postedFile != null && postedFile.Length > 0)
                {
                    ...
                    {
                        if (!string.IsNullOrEmpty(dt.Rows[i]["casetype"].ToString()))
                        {
                            tbl_casetype obj = new tbl_casetype()
                            {
                                cmp_id = cmpid,
                                casetype = dt.Rows[i]["casetype"].ToString(),
        ...
        {
            var path = Path.Combine(Environment.WebRootPath + "/Uploads/Sample", "casetype.xlsx");
            var fs = new FileStream(path, FileMode.Open);

            // Return the file. A byte array can also be used instead of a stream
            return File(fs, "application/octet-stream", "Samplecasetype.xlsx");
        }
        ...
        public IActionResult ExportToExcel()
        {
            try
            {
                ...
                foreach (var casetype in data)
                {
                    dt.Rows.Add(casetype.casetype);
                }
                ...
                using (var document = SpreadsheetDocument.Create(memoryStream, SpreadsheetDocumentType.Workbook))
                {
                    ...
                    var sheet = new Sheet() { Id = document.WorkbookPart.GetIdOfPart(worksheetPart), SheetId = 1, Name = "CaseType" };
                    sheets.Append(sheet);
                ...
                return File(memoryStream.ToArray(), "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", "CaseType.xlsx");
            }
            catch (Exception ex)
            {
